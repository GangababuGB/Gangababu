...
r_df = resampled_df.mean().reset_index() # sums qty repeated date
...
title_csv = '-r_df_W_IMP.csv'
Label = 'IQR_RC_WEEK-WISE_'
# r_dfw = r_df2.resample('5D')
r_dfw = FP.resample('5D')
# r_dfw = r_dfw.sum().reset_index()
r_dfw = r_dfw.mean().reset_index()# means qty repeated date
...
# -----------------------------------------------------------
#
r_dfw.to_csv('F:/GangaBabu/20.01.2022/5D Resample/5D_data/'+Label+sel_p[0]+title_csv)
# print(r_dfw.head(10))
