...
with open('Mean_WEEK-WISE_FGAS-660A-16-00087_BC.csv') as csvfile:
...
split_time = int(0.94 * len(series))
print('series: ', len(series))
print('split:', split_time)
print('split len: ', len(series)-split_time)
...
rho=0.94
# opt = tf.keras.optimizers.RMSprop(learning_rate=lr, rho=rho, epsilon=1e-08, decay=0.0)
...
model.fit(dataset, epochs=epochs, callbacks=[callback], verbose=1)
model.save("fc87_model")
# # x_train FC
forecast = []
...
#   forecast.append(model.predict(x_train[i:i + window_size][np.newaxis]))
...
# #-------------------temp plot
time_d = pd.date_range(start='04/07/2019', periods=51, freq='W-SUN')
...
Final_df = pd.concat([time_d, x_train, fc], axis=1)
Final_df.columns = ['Date', 'x_train', 'PRED_FC']
# Final_df.set_index('Date', inplace=True)
print(Final_df.tail())

# plt.plot(Final_df['Date'], series, label='series')
plt.plot(Final_df['Date'], Final_df['x_train'], label='Actual')
plt.plot(Final_df['Date'], Final_df['PRED_FC'], label='Forecast')
plt.title('KPL time series prediction - Actual Vs. Predicted & Forecast [ MAE:'+str(MAE)+
